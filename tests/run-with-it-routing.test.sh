...
assert_file_line_contains "$SKILL_FILE" '| `RUN_WITH_IT_EXPLICIT_LEGACY_OVERRIDES` |' '`AGENT`, `MODEL`, or both' "skill documents the per-alias legacy provenance marker"
assert_file_section_contains "$SKILL_FILE" '══ STEP C: ASSEMBLE SUB-COORDINATOR CONTEXT FILES ══════════════════════════════' '  The Sub-Coordinator must derive a separate' 'RUN_WITH_IT_EXPLICIT_LEGACY_OVERRIDES=AGENT,MODEL' "Step C shows the explicit legacy provenance channel"
assert_file_contains "$ORCHESTRATOR_RULES_FILE" '`RUN_WITH_IT_EXPLICIT_LEGACY_OVERRIDES`' "Main Orchestrator rules require the explicit legacy provenance marker"
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" 'The dispatcher removes `AGENT`, `MODEL`, and the provenance marker before launching this Sub-Coordinator.' "Sub-Coordinator contract records dispatcher-boundary scrubbing"
assert_file_line_contains "${ROOT_DIR}/README.md" '| `RUN_WITH_IT_EXPLICIT_LEGACY_OVERRIDES` |' 'Per-alias provenance marker' "README documents the legacy provenance marker"
