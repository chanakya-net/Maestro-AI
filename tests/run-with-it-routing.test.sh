...
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" 'Each `in_flight_agents` entry must include `role`, `cycle`, `pid`, `agent`, `model`, `effort`, `selection_reason`' "sub-state in-flight schema preserves effort and selection reason"
...
assert_file_not_contains "$SKILL_FILE" 'Reset ALL of them to `status="pending"`' "resume flow no longer resets in_progress issues unconditionally"
...
assert_file_contains "$SKILL_FILE" 'STATUS|type=discard-shutdown' "discard documents the identity-checked shutdown status line"
...
assert_file_contains "${ROOT_DIR}/README.md" 'GPT-5.5, GPT-5.6 Sol, Codex Spark, Claude Sonnet 5' "README documents medium-hard models"
assert_file_contains "${ROOT_DIR}/README.md" 'GPT-5.6 Sol, Claude Opus 4.8' "README documents complex models"
...
assert_file_contains "$SKILL_FILE" 'Sonnet 5: `low`, `medium`, `medium`, `high`' "skill documents Sonnet effort"
assert_file_contains "$SKILL_FILE" 'Opus 4.8: `xhigh` at complex, `max` at holy-fuck' "skill documents Opus effort"
cmp -s "$SKILL_FILE" "${ROOT_DIR}/.agents/skills/run-with-it/SKILL.md" || fail "run-with-it skill mirrors differ"
...
assert_file_contains "$REVIEW_PROMPT_FILE" 'coverage_matrix' "review prompt requires structured coverage matrix"
assert_file_contains "$REVIEW_PROMPT_FILE" 'Threat Model Pass' "review prompt requires explicit security threat-model pass"
...
assert_file_contains "$MODIFIER_PROMPT_FILE" 'review_comment_closure' "modifier prompt requires review comment closure ledger"
