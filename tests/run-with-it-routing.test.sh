...
assert_file_section_contains "$SKILL_FILE" 'Shared required files:' 'Bash required helper files:' '- `python/run-with-it-pr-body.py`' "shared required file list includes PR body renderer"
...
assert_contains '`RUN_WITH_IT_HELPER_RUNTIME` is `python` (default) or `csharp`' "preflight documents helper runtime selector"
assert_contains 'DOTNET_BIN' "preflight documents C# helper runtime"
assert_contains '`DOTNET_BIN` | `dotnet`' "helper runtime docs include DOTNET_BIN"
assert_contains 'requires .NET SDK 10+' "helper runtime docs include C# minimum version"
...
assert_file_contains "$ORCHESTRATOR_RULES_FILE" 'scripts/worker-watch.sh' "orchestrator rules use worker-watch for sub-coordinator liveness"
...
assert_file_contains "$ORCHESTRATOR_RULES_FILE" 'do not use raw UUID branches' "orchestrator rules forbid UUID run branches"
...
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" '--prompt-file "$ASSET_ROOT/prompts/prompt.md"' "sub-coordinator uses nested prompts/prompt.md for impl worker"
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" '--prompt-file "$ASSET_ROOT/prompts/review-prompt.md"' "sub-coordinator uses nested prompts/review-prompt.md for review worker"
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" '--prompt-file "$ASSET_ROOT/prompts/modifier-prompt.md"' "sub-coordinator uses nested prompts/modifier-prompt.md for modifier worker"
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" '--prompt-file "$ASSET_ROOT/prompts/complexity-prompt.md"' "sub-coordinator uses nested prompts/complexity-prompt.md for complexity worker"
...
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" 'Maestro/cunning-fox' "sub-coordinator examples use Maestro branch naming"
...
assert_file_contains "$SUB_COORDINATOR_PROMPT_FILE" 'scripts/worker-watch.sh' "sub-coordinator uses worker-watch helper"
