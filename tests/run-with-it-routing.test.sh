...
assert_contains 'Hard Minimum Rules' "documents hard minimum rules"
assert_contains 'Capability Bands' "documents capability bands"
assert_contains '`medium-hard|complex|holy-fuck` => `advanced`' "documents advanced capability mapping"
assert_contains '`quite-easy|easy`: `gemini`, then `github-copilot`, then `claude`, then `codex`' "documents Gemini preference for easy tasks"
assert_contains 'For trivial file read or concatenate-style tasks such as `cat`, choose `gemini` first' "documents Gemini preference for cat tasks"
...
assert_contains 'ROUTE|agent=<agent>|model=<model>|complexity_level=<level>|complexity_score=<score>|required_capability=<band>|fallback_budget=<n>|allowlist=<value>|denylist=<value>' "documents parseable route line"
