...
assert_contains 'the initial issue body must use the exact Markdown template headings below in the exact order shown' "documents strict ordered issue body template"
assert_contains '1. `## Parent`' "documents Parent section order"
assert_contains '2. `## What to build`' "documents What to build section order"
assert_contains '3. `## Implementation Steps`' "documents Implementation Steps section order"
assert_contains '4. `## Agent Routing`' "documents Agent Routing section order"
assert_contains '5. `## Technical Context Snapshot`' "documents Technical Context Snapshot section order"
assert_contains '6. `## Acceptance criteria`' "documents Acceptance criteria section order"
assert_contains '7. `## Blocked by`' "documents Blocked by section order"
assert_contains 'keep each implementation issue nearly identical to the GitHub issue body format' "documents local fallback parity with GitHub issue body"
