...
assert_contains 'Build a project orientation snapshot before root-cause claims:' "requires project architecture orientation before diagnosis"
assert_contains 'Map internal dependencies: repo modules/packages, shared utilities, generated code, schemas/migrations, data stores, queues/jobs, services, configuration, feature flags, and test/build tooling that participate in the failure path.' "requires internal dependency mapping"
assert_contains 'Map external dependencies: third-party packages, SDKs, APIs, SaaS services, infrastructure services, auth/payment/storage providers, network boundaries, environment variables, and version constraints that participate in the failure path.' "requires external dependency mapping"
assert_contains 'Do not treat architecture or dependencies as background context; capture only the parts that explain the failure or bound the fix.' "keeps architecture/dependency capture issue-focused"
...
assert_contains 'Expected vs Actual: clear comparison of what should happen and what happens instead.' "requires clearer human issue explanation"
assert_contains 'Architecture and Dependency Summary: human-readable overview of the relevant architecture, internal dependencies, and external dependencies.' "requires human-readable architecture/dependency summary"
...
assert_contains '`Project Overview`' "requires project overview section"
assert_contains '`Architecture Map`' "requires architecture map section"
assert_contains '`Internal Dependencies`' "requires internal dependencies section"
assert_contains '`External Dependencies`' "requires external dependencies section"
...
assert_contains '`File-by-File Change Guide`' "requires file-by-file implementation guide"
...
assert_contains '`Acceptance Criteria`' "requires acceptance criteria section"
assert_contains 'Use deterministic, model-independent instructions: write exact file paths, symbols, expected behavior, preconditions, postconditions, and validation commands; avoid vague phrases like "handle this better" unless immediately followed by the precise required behavior.' "requires explicit cross-LLM handoff instructions"
assert_contains 'Any implementation LLM should be able to apply the fix using this file without reading the chat history.' "requires standalone LLM handoff context"
