...
assert_contains_file "$SKILL_FILE" '(Join-Path $ASSET_ROOT "powershell/run-with-it-pool.ps1")' "skill uses nested PowerShell pool runner path"
assert_contains_file "$AGENT_SKILL_FILE" '(Join-Path $ASSET_ROOT "powershell/run-with-it-pool.ps1")' "agent skill mirror uses nested PowerShell pool runner path"
