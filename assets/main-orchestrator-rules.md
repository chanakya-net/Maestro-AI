...
- When `PARALLEL_JOBS > 1`: the pool runner keeps up to that many compatible dispatch processes active and fills freed slots immediately. Persist `parallel_safe` and normalized `ownership_scope` for every issue. Issues run in isolated per-issue worktrees, so missing metadata admits in parallel; only an explicit `parallel_safe=false` or a proven `ownership_scope` overlap defers an issue, and the pool runner surfaces deferrals as `STATUS|type=pool-admission-deferred`. Each issue has its own context file, log file, done file, and report file.
