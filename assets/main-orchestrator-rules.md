...
- Pass `FORCED_AGENT` and `FORCED_MODEL` only for explicit user-requested child-worker overrides. Normalize deprecated `AGENT` and `MODEL` aliases only when explicitly supplied by the user, and set `RUN_WITH_IT_EXPLICIT_LEGACY_OVERRIDES` to the exact comma-separated alias names (`AGENT`, `MODEL`, or `AGENT,MODEL`) on the pool/dispatcher invocation. Omit the marker for ambient aliases. Canonical `FORCED_*` values win; the dispatcher consumes the marker and scrubs `AGENT` and `MODEL` before launch, so ambient aliases remain runner telemetry rather than routing policy.
...
- When `PARALLEL_JOBS > 1`: the pool runner keeps up to that many compatible dispatch processes active and fills freed slots immediately. Persist `parallel_safe` and normalized `ownership_scope` for every issue. Missing metadata or `parallel_safe=false` is exclusive; explicitly safe issues may overlap only when scopes are disjoint. Each issue has its own context file, log file, done file, and report file.
