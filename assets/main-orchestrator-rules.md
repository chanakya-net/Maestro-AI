...
- Never treat starting the pool process as completion of the Main Coordinator turn. Run the pool runner in the foreground; if the execution tool yields a live terminal/session identifier, resume that same session until the pool exits after emitting `STATUS|type=pool-empty`.
