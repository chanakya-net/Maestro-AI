# run-codex.ps1 — Windows PowerShell runner for Codex (mirrors run-codex.sh)
#
# Usage:
#   run-codex.ps1 <context-payload-file> [prompt-file]
#
# Environment variables:
#   COPY_PROMPT, PRINT_PROMPT, CODEX_PERMISSION_MODE, CODEX_EXTRA_ARGS,
...
$CODEX_PERMISSION_MODE    = if ($env:CODEX_PERMISSION_MODE)              { $env:CODEX_PERMISSION_MODE }    else { "--dangerously-bypass-approvals-and-sandbox" }
$CODEX_EXTRA_ARGS         = if ($env:CODEX_EXTRA_ARGS)                   { $env:CODEX_EXTRA_ARGS }         else { "" }
...
$AGENT_NAME_PREFIX        = if ($env:AGENT_NAME_PREFIX)                  { $env:AGENT_NAME_PREFIX }        else { "codex-agent" }
...
if (-not (Get-Command "codex" -ErrorAction SilentlyContinue)) {
    Write-Error "Missing required command: codex"
    exit 1
}
...
try {
    ...
    for ($iteration = 1; $iteration -le $MAX_ITERATIONS; $iteration++) {
        ...
        Write-Host "== Codex iteration ${iteration}/${MAX_ITERATIONS} [$runName] ==" -ForegroundColor Gray
        ...
        $cmdArgs = @("exec", "-C", $REPO_ROOT)
        if ($CODEX_PERMISSION_MODE) { $cmdArgs += $CODEX_PERMISSION_MODE }
        if ($CODEX_EXTRA_ARGS)      { $cmdArgs += $CODEX_EXTRA_ARGS }
        $cmdArgs += $payloadText
