...
- The platform dispatcher auto-fails stalled roles listed in `RUN_WITH_IT_AUTO_FAIL_STALLED_ROLES` (default: `complexity,impl,modify`) by terminating the stalled runner and emitting `STATUS|type=worker-stall-timeout` followed by `dispatch-failed`. Complexity stalls consume the complexity fallback path; implementation and modification stalls consume the worker artifact recovery path instead of blocking the pool forever.
...
- The default silence thresholds are `WORKER_QUIET_SECONDS=120` and `WORKER_STALL_SECONDS=600`. Before auto-failing an `alive-but-silent` stalled implementation/modification worker, the dispatcher first salvages any work it left behind — a committed-but-unreported HEAD advance, or an uncommitted dirty tree it commits — and only fails the worker when there is genuinely no git progress. After a stalled state with no salvageable work, follow the role's failure/fallback rule or alert the user if no automatic fallback is safe.
