...
function Install-Assets {
    ...
    if ($DryRun) {
        ...
        foreach ($f in $scripts) {
            Note "  [dry-run] Invoke-WebRequest $baseUrl/powershell/$f -OutFile $ASSETS_DEST\powershell\$f"
        }
        foreach ($f in $python) {
    ...
    foreach ($f in $scripts) {
        $url  = "$baseUrl/powershell/$f"
        $tmp  = "$ASSETS_DEST\powershell\$f.tmp"
        $dest = "$ASSETS_DEST\powershell\$f"
        try {
            Invoke-WebRequest -Uri $url -OutFile $tmp -ErrorAction Stop
            Move-Item -Force $tmp $dest
        }
    ...
    $url = "$baseUrl/agent-registry.json"
    $tmp = "$ASSETS_DEST\agent-registry.json.tmp"
    try {
        Invoke-WebRequest -Uri $url -OutFile $tmp -ErrorAction Stop
        Move-Item -Force $tmp "$ASSETS_DEST\agent-registry.json"
    }
    catch {
        Remove-Item -Force $tmp -ErrorAction SilentlyContinue
        $FAILED.Add("assets")
